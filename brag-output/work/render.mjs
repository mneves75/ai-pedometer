...
const URL0 = `http://127.0.0.1:${server.address().port}/${PAGE}`;

const browser = await chromium.launch({ executablePath: process.env.HOME + "/Library/Caches/ms-playwright/chromium_headless_shell-1234/chrome-headless-shell-mac-arm64/chrome-headless-shell", args: ["--force-color-profile=srgb", "--disable-lcd-text", "--font-render-hinting=none"] });
...
if (mode === "stills") {
  const dir = path.join(ROOT, process.env.STILLS ?? "stills"); fs.mkdirSync(dir, { recursive: true });
  const { shot } = await openPage();
  for (const t of arg.split(",").map(Number)) fs.writeFileSync(path.join(dir, `t-${t.toFixed(2)}.png`), await shot(t));
  console.log("stills ok");
} else if (mode === "video") {
  const fps = Number(fpsArg ?? 60), sub = Number(subArg ?? 1), W = Number(workersArg ?? 4);
  const total = Math.round(21 * fps);
  const segDir = path.join(ROOT, process.env.SEGS ?? "segs"); fs.mkdirSync(segDir, { recursive: true });
  ...
  await Promise.all(Array.from({ length: W }, async (_, w) => {
    const a = Math.floor((total * w) / W), b = Math.floor((total * (w + 1)) / W);
    const { shot } = await openPage();
    ...
    const ff = spawn("ffmpeg", ["-y", "-loglevel", "error", "-f", "image2pipe", "-c:v", "png", "-framerate", String(fps * sub), "-i", "-",
      "-vf", vf, "-r", String(fps), "-c:v", "libx264", "-preset", "medium", "-crf", "10", "-pix_fmt", "yuv420p", "-color_primaries", "bt709", "-color_trc", "bt709", "-colorspace", "bt709", out], { stdio: ["pipe", "inherit", "inherit"] });
    for (let f = a; f < b; f++) {
      for (let s = 0; s < sub; s++) {
        const t = (f + (sub > 1 ? (s / sub - .5) * .5 : 0)) / fps; // −¼ a +¼ de quadro
        const buf = await shot(Math.max(0, t));
        if (!ff.stdin.write(buf)) await new Promise((r) => ff.stdin.once("drain", r));
      }
      if (w === 0 && f % 30 === 0) console.log(`w0 ${f - a}/${b - a} ${((Date.now() - t0) / 1000).toFixed(0)}s`);
    }
    ff.stdin.end();
    await new Promise((r, j) => ff.on("close", (c) => (c ? j(new Error("ffmpeg " + c)) : r())));
  }));
  fs.writeFileSync(path.join(segDir, "list.txt"), Array.from({ length: W }, (_, w) => `file 'seg${w}.mp4'`).join("\n"));
  await new Promise((r, j) => spawn("ffmpeg", ["-y", "-loglevel", "error", "-f", "concat", "-safe", "0", "-i", path.join(segDir, "list.txt"), "-c", "copy", arg], { stdio: "inherit" }).on("close", (c) => (c ? j(new Error("concat")) : r())));
