...
- Shared 32-bit code: build `AIPedometerWatch` for generic watchOS with `ARCHS=arm64_32`, `ONLY_ACTIVE_ARCH=YES` and `CODE_SIGNING_ALLOWED=NO`. An iOS or watch simulator build is insufficient.
