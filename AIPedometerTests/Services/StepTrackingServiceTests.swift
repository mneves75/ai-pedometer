...
final class MockHealthKitService: HealthKitServiceProtocol, Sendable {
    ...
    var fetchDistanceHandler: (@MainActor @Sendable (Date, Date) async throws -> Double)?
    var fetchFloorsHandler: (@MainActor @Sendable (Date, Date) async throws -> Int)?
    var fetchLatestHeartRateHandler: (@MainActor @Sendable (Date, Date) async throws -> HeartRateSample?)?
    ...
    func fetchDistance(from startDate: Date, to endDate: Date) async throws -> Double {
        ...
        if let fetchDistanceHandler {
            return try await fetchDistanceHandler(startDate, endDate)
        }
    ...
    func fetchFloors(from startDate: Date, to endDate: Date) async throws -> Int {
        ...
        if let fetchFloorsHandler {
            return try await fetchFloorsHandler(startDate, endDate)
        }
    ...
    func fetchLatestHeartRateSample(from startDate: Date, to endDate: Date) async throws -> HeartRateSample? {
        ...
        if let fetchLatestHeartRateHandler {
            return try await fetchLatestHeartRateHandler(startDate, endDate)
        }
...
private actor ConcurrentQueryProbe {
...
private func makeService(
    ...
    goalService injectedGoalService: GoalService? = nil,
    calculator: DailyStepCalculator = DailyStepCalculator(),
    now: @escaping @MainActor () -> Date = { .now }
) -> (service: StepTrackingService, goalService: GoalService) {
    let goalService = injectedGoalService ?? GoalService(persistence: persistence)
    if let goalValue {
...
struct StepTrackingServiceTests {
    ...
    @Test("Independent today metrics queries overlap within one refresh")
    @MainActor
    func todayMetricsQueriesRunConcurrently() async {
        let mockHealthKit = MockHealthKitService()
        let mockMotion = MockMotionService()
        let probe = ConcurrentQueryProbe()
        mockHealthKit.stepsToReturn = 8_765
        mockHealthKit.fetchDistanceHandler = { _, _ in
            await probe.hold()
            return 6_000
        }
        mockHealthKit.fetchFloorsHandler = { _, _ in
            await probe.hold()
            return 4
        }
        mockHealthKit.fetchLatestHeartRateHandler = { _, _ in
            await probe.hold()
            return HeartRateSample(bpm: 72, endDate: .now)
        }
        let testDefaults = TestUserDefaults()
        defer { testDefaults.reset() }
        testDefaults.defaults.set(
            DistanceEstimationMode.automatic.rawValue,
            forKey: AppConstants.UserDefaultsKeys.distanceEstimationMode
        )
        let (service, _) = makeService(
            healthKit: mockHealthKit,
            motion: mockMotion,
        ...
        #expect(await probe.maxActiveCount == 3)
        #expect(service.todaySteps == 8_765)
        #expect(service.todayDistance == 6_000)
        #expect(service.todayFloors == 4)
        #expect(service.todayHeartRateBPM == 72)
    }
    ...
    @Test("Failed goal save keeps the durable goal in service and shared data")
    @MainActor
    func failedGoalSaveDoesNotPublish() async {
        let mockHealthKit = MockHealthKitService()
        let mockMotion = MockMotionService()
        let testDefaults = TestUserDefaults()
        defer { testDefaults.reset() }
        let persistence = PersistenceController(inMemory: true)
        GoalService(persistence: persistence).setGoal(9_000)
        let failingGoalService = GoalService(
            persistence: persistence,
            saveModelContext: { _ in throw HealthKitError.queryFailed }
        )
        ...
        service.updateGoal(12_000)

        #expect(service.currentGoal == 9_000)
        #expect(failingGoalService.currentGoal == 9_000)
        #expect(testDefaults.defaults.sharedStepData?.goalSteps == 9_000)
    }
