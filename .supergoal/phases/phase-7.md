...
- Repo: ~/dev/PROJETOS_MOBILE/ai-pedometer.
...
- `AGENTS.md` contains a `## GUIDELINES-REF` section that is diff-checked against `~/dev/GUIDELINES-REF/AGENTS.md` by `Scripts/check-agents-sync.sh` — all edits must stay ABOVE that marker and the gate must stay green. Do not edit the synced section.
