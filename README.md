...
```bash
asc validate --app "<APP_ID_ASC>" --version "1.0.2" --platform IOS --output table
...
**Current source**: 1.0.2 (58). Source version does not indicate TestFlight or App Store availability. Production delivery is currently blocked on an Apple (`appl_`) RevenueCat key — see CHANGELOG 1.0.
