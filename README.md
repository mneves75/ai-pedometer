...
```bash
asc validate --app "<APP_ID_ASC>" --version "1.0.4" --platform IOS --output table
...
**Current source**: 1.0.4 (60). Source version does not indicate TestFlight or App Store availability. Production delivery still requires an Apple (`appl_`) RevenueCat configuration, signed artifacts and physical-device acceptance. The 1.0.4 code review does not publish a release or validate real payments.
